...
public class UnicodeInputStreamReaderTest extends TestIni4jCase {
  ...
  private void test(String filename, String defaultEncoding) throws Exception {
    Charset charset =
        (defaultEncoding == null) ? Config.DEFAULT_FILE_ENCODING : Charset.forName(defaultEncoding);
    UnicodeInputStreamReader reader =
        new UnicodeInputStreamReader(getClass().getResourceAsStream(filename), charset);
    Ini ini = new Ini();
