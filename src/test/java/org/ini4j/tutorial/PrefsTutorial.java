...
// |
// | As soon as the Preferences object is created it functions as a standard Preferences node, and
// should be
// | used as such. Implicitly only new nodes can be created in the root node (these will be the
// sections).
// | In the first level nodes (sections) only values can be created (these will be the options).
// |
// | In the case of an invalid operation an <<<UnsupportedOperationException>>> type runtime
